function make_record_card() {
  ...
  const sheet_id =
    PropertiesService.getScriptProperties().getProperty("sheet_id"); // get sheet_id
  ...
  const diff =
    Math.floor(
      (nowDate.getTime() - start_date.getTime()) / (1000 * 60 * 60 * 24)
    ) + 1; // days between now and start_date
  ...
  const reply_contents = card_json(
    postDate,
    menstruation_days,
    diff,
    pill_color,
